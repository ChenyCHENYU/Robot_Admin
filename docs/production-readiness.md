...
- 架构保持单体 SPA 和既有路由、状态管理边界不变，业务组件继续通过 `@robot-admin/naive-ui-components@0.11.7` 按需消费；每个组件样式入口均包含自身组合组件所需样式，不再依赖全量 CSS 兜底。
...
| module preload      | 582.06 KiB | 486.22 KiB | -16.5%   | ≤ 650 KiB  |
