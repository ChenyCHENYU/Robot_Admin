...
| 包名                                                                                                 | 版本   | 作用          | 状态        |
| ---------------------------------------------------------------------------------------------------- | ------ | ------------- | ----------- |
| [`@robot-admin/naive-ui-components`](https://www.npmjs.com/package/@robot-admin/naive-ui-components) | 0.11.6 | 业务组件库    | ✅ 活跃维护 |
| [`@robot-admin/request-core`](https://www.npmjs.com/package/@robot-admin/request-core)               | 0.5.0  | 请求核心库    | ✅ 活跃维护 |
| [`@robot-admin/layout`](https://www.npmjs.com/package/@robot-admin/layout)                           | 3.2.1  | 布局系统      | ✅ 活跃维护 |
| [`@robot-admin/theme`](https://www.npmjs.com/package/@robot-admin/theme)                             | 0.5.1  | 主题系统      | ✅ 活跃维护 |
| [`@robot-admin/directives`](https://www.npmjs.com/package/@robot-admin/directives)                   | 2.0.1  | 自定义指令集  | ✅ 活跃维护 |
| [`@robot-admin/form-validate`](https://www.npmjs.com/package/@robot-admin/form-validate)             | 3.4.2  | 表单验证库    | ✅ 活跃维护 |
| [`@robot-admin/file-utils`](https://www.npmjs.com/package/@robot-admin/file-utils)                   | 3.0.1  | 文件工具集    | ✅ 活跃维护 |
| [`@robot-admin/git-standards`](https://www.npmjs.com/package/@robot-admin/git-standards)             | 1.0.5  | Git标准化工具 | ✅ 活跃维护 |
...
```json
{
  ...
  "md-editor-v3": "6.5.6", // C_Markdown 底层编辑器
...
- 实例隔离的 Axios 请求编排、缓存、取消、去重和重试
- Token 主动刷新、并发 401 恢复与重新登录 single-flight
- UI 无关的 `useRequest`、`useTableCrud` 与 `createTableCrud`
- 函数式单表预配置和可覆盖的分页参数映射
- 标准化请求错误和多种后端响应结构提取
...
const request = createRequestClient({
  request: { baseURL: '/api' },
  auth: { getToken, refresh, reauthenticate },
})
...
**版本：** 3.2.1（Naive 单入口、Vue Headless 分层，保留 3.x 兼容层）

**核心能力：**

- 布局系统与设置管理
- 6 种布局模式与 Pinia 集成
- `provideLayout()` 精简宿主接入
- Naive UI 项目统一使用 `@robot-admin/layout/naive`，`C_*Layout` 为唯一推荐组件命名
- `@robot-admin/layout/core` 与 `/vue` 提供无 UI 协议和 Headless 复用边界
- 主题同步、安全扩展插槽与宿主副作用边界
...
- Naive UI / Element Plus 双框架统一规则源
- 框架无关批量校验与嵌套字段路径
- 条件、组合、防抖异步和数据库数值契约验证
- 配置隔离、完整 TypeScript 推导与中国本地化支持
