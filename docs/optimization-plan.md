# 🔧 全局组件优化计划

> **分支**：`perf/C_Table`  
> **架构模式**：薄 UI 壳 + 厚 Composable 引擎  
> **最后更新**：2026-02-16

## 已完成 ✅

| 组件               | 优化前行数 | 优化后行数 | 提取的 Composable                                                                                                                                                                   | 阶段    |
| ------------------ | ---------- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| **C_Table**        | ~1200      | ~300       | `useTableConfig` · `useTableManager` · `useTableColumns` · `useTableActions` · `usePagination` · `useRowEdit` · `useCellEdit` · `useModalEdit` · `useDynamicRow` · `useTableExpand` | Phase 0 |
| **C_Form**         | ~711       | ~308       | `useFormConfig` · `useFormState` · `useFormRenderer`                                                                                                                                | Phase 0 |
| **C_FormSearch**   | 428        | ~230       | `useSearchHistory` · `useSearchState`                                                                                                                                               | Phase 1 |
| **C_Draggable**    | 554        | ~170       | `useDraggableLayout`                                                                                                                                                                | Phase 5 |
| **C_Tree**         | 524        | ~168       | `useTreeOperations`                                                                                                                                                                 | Phase 5 |
| **C_GlobalSearch** | ~380       | ~230       | `useGlobalSearch`                                                                                                                                                                   | Phase 5 |
| **C_FullCalendar** | ~340       | ~165       | `useCalendarEvents`                                                                                                                                                                 | Phase 5 |
| **C_Time**         | ~310       | ~115       | `useTimeSelection`                                                                                                                                                                  | Phase 5 |

---

## 待优化组件清单

### 🔴 P0 — 高优先级

#### 1. C_FilePreview

| 项目     | 详情                                                                                                                                                                                                                                 |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 当前行数 | **816 行**（index.vue）+ 378 行（data.ts）+ 484 行（index.scss）                                                                                                                                                                     |
| 核心问题 | 单文件处理 PDF / Word / Excel / 图片 / 视频 / 音频 / 文本 7 种预览逻辑；UI 状态变量散乱（`currentPage`、`totalPages`、`scale`、`wordZoom`、`showOutline`、`activeSheet`…）；全部预览逻辑内联无提取；模板过长                         |
| 优化方向 | ① 按文件类型拆分子组件：`PdfViewer` / `WordViewer` / `ExcelViewer` / `ImageViewer` / `MediaViewer` / `TextViewer`<br>② 提取 `useFilePreview` composable（文件类型判定 + 通用状态）<br>③ 类型迁移到 `types/modules/file-preview.d.ts` |
| 目标     | index.vue → ~200 行薄壳 + 6 个子组件 + 1 个 composable                                                                                                                                                                               |

#### 2. C_AntV（ER / UML / BPMN 子布局）

| 项目     | 详情                                                                                                                                                          |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 当前行数 | ER: **710**、UML: **696**、BPMN: **570**（共 ~1976 行核心代码）                                                                                               |
| 核心问题 | 三个子布局高度重复：toolbar、drawer 编辑器、graph 初始化、主题处理、导出逻辑；父组件有 `console.log` 调试残留                                                 |
| 优化方向 | ① 提取 `useGraphEditor` / `useGraphToolbar` / `useGraphExport` 共享 composable 消除重复<br>② 统一类型定义到 `types/modules/antv.d.ts`<br>③ 清理 `console.log` |
| 目标     | 每个子布局 → ~250 行，消除 ~60% 重复代码                                                                                                                      |

#### 3. C_WorkFlow

| 项目     | 详情                                                                                                                            |
| -------- | ------------------------------------------------------------------------------------------------------------------------------- |
| 当前行数 | **721 行**（index.vue）+ 115 行（data.ts）+ 4 个节点子组件                                                                      |
| 核心问题 | 节点操作逻辑（add/delete/connect/validate）全部内联；`provide('showAddMenu')` / `provide('deleteNode')` 使用粗糙                |
| 优化方向 | ① 提取 `useWorkflowNodes` composable（节点 CRUD + provide/inject）<br>② 提取 `useWorkflowValidation` composable（流程校验逻辑） |
| 目标     | index.vue → ~300 行 + 2 个 composable                                                                                           |

#### ~~4. C_Draggable~~ ✅ 已完成

| 项目   | 详情                                                                                |
| ------ | ----------------------------------------------------------------------------------- |
| 优化前 | **554 行**（index.vue）+ 313 行（index.scss）                                       |
| 优化后 | **~170 行**（index.vue）+ 257 行（useDraggableLayout.ts）+ 110 行（draggable.d.ts） |
| 提取   | ① 类型迁移 `types/modules/draggable.d.ts`<br>② `useDraggableLayout` composable      |
| 瘦身率 | **-69%**（554 → 170 行）                                                            |

---

### 🟠 P1 — 中优先级

| 组件                   | 行数               | 核心问题                                       | 优化方向                                                     |
| ---------------------- | ------------------ | ---------------------------------------------- | ------------------------------------------------------------ |
| ~~**C_Tree**~~         | ~~523~~            | ~~render 函数 + 树操作逻辑全内联~~             | ✅ 已提取 `useTreeOperations` composable + `data.ts`         |
| ~~**C_GlobalSearch**~~ | ~~494 + 742 SCSS~~ | ~~键盘导航、搜索历史、菜单扁平化全内联~~       | ✅ 已提取 `useGlobalSearch` composable + `globalSearch.d.ts` |
| ~~**C_FullCalendar**~~ | ~~496~~            | ~~老式 runtime defineProps；硬编码颜色/中文~~  | ✅ type-based defineProps + `useCalendarEvents` + `data.ts`  |
| ~~**C_Time**~~         | ~~394~~            | ~~过度注释；多个 expose getter/setter 可简化~~ | ✅ 已提取 `useTimeSelection` + ref-based expose              |

| **C_Editor** | 390 | 手动事件监听、setTimeout 初始化、`console.log` 残留 | VueUse 事件处理 + 清理调试代码 |
| **C_VtableGantt** | 382 + 528 data.ts | 自定义 `deepMerge` 内联在组件中 | `deepMerge` → utils 提取 |
| **C_Code** | 369 | 硬编码 `languageIcon`(24项) + `getLanguageTitle`(24项) | 数据移到 data.ts |
| **C_City** | 368 + 4813 city.ts | 手动 `clickOutside` 处理 | 改用 VueUse `onClickOutside` |
| **C_Icon** | 365 | 5 种图标模式验证规则内联 | 规则 map 提取到 data.ts |
| **C_ActionBar** | 364 | 混合模板 + `h()` 渲染函数风格 | 统一渲染风格 |
| **C_Date** | 360 | 5 种日期模式重复 handler | handler 合并简化 |
| **C_TagsView** | 346 | 直接操作 localStorage；手动 DOM 滚动 | 通过 store 管理 + VueUse |

---

### 🟡 P2 — 低优先级（结构尚可，小改即可）

| 组件       | 行数 | 问题                                | 改动                       |
| ---------- | ---- | ----------------------------------- | -------------------------- |
| C_Map      | 293  | `console.log` + `setTimeout` 初始化 | 清理调试代码               |
| C_Notice   | 287  | 全部 mock 数据                      | 接入真实 API               |
| C_Menu     | 273  | 递归查找逻辑可优化                  | 小幅重构                   |
| C_Markdown | 272  | 存根方法未实现                      | 补全或标注 TODO            |
| C_Progress | 240  | `setInterval` 动画                  | 改用 requestAnimationFrame |
| C_Captcha  | 183  | 结构清晰                            | 无需改动                   |
| C_Cascade  | 155  | 内联 scoped style                   | 小改                       |

---

### 🟢 P3 — 无需优化

| 组件            | 行数 | 评价                          |
| --------------- | ---- | ----------------------------- |
| C_Layout        | 97   | 唯一使用 composable 的组件 ✅ |
| C_Steps         | 110  | 简洁，导出类型 ✅             |
| C_Barcode       | 108  | 简洁清晰 ✅                   |
| C_MenuTop       | 105  | ResizeObserver 合理 ✅        |
| C_Header        | 98   | 协调组件 ✅                   |
| C_Breadcrumb    | 84   | 简洁 ✅                       |
| C_Guide         | 87   | 可接受 ✅                     |
| C_Language      | 74   | 轻量 ✅                       |
| C_Theme         | 67   | 使用 store ✅                 |
| C_ReLoginDialog | 170  | 结构清晰 ✅                   |
| C_NavbarRight   | 164  | 协调组件 ✅                   |
| C_Settings      | 24   | 纯包装器 ✅                   |
| C_Footer        | 21   | 最简 ✅                       |

---

## 全局 Code Smell 统计

| Code Smell                | 涉及组件数 | 说明                                                         |
| ------------------------- | ---------- | ------------------------------------------------------------ |
| **无 composable 提取**    | 34/38      | 仅 C_Layout、C_Table、C_Form、C_FormSearch 使用了 composable |
| **内联类型定义**          | ~30/38     | 仅 C_ActionBar、C_WorkFlow、C_FormSearch 从 types/ 导入      |
| **大文件 > 300 行**       | **16 个**  | 详见上方分级                                                 |
| **console.log 残留**      | ~8 个      | C_AntV、C_Editor、C_Map、C_Markdown 等                       |
| **硬编码中文字符串**      | ~15 个     | 未走 i18n                                                    |
| **手动 addEventListener** | ~6 个      | 应使用 VueUse `useEventListener`                             |
| **setTimeout 初始化**     | ~5 个      | 应改用 `nextTick` 或 `await`                                 |

---

## 推荐执行路线图

| 阶段        | 组件                                                                | 预估工作量 | 状态      |
| ----------- | ------------------------------------------------------------------- | ---------- | --------- |
| Phase 0     | C_Table + C_Form                                                    | —          | ✅ 已完成 |
| Phase 1     | C_FormSearch                                                        | 0.5 天     | ✅ 已完成 |
| Phase 2     | C_FilePreview                                                       | 2 天       | ✅ 已完成 |
| Phase 3     | C_AntV (ER/UML/BPMN)                                                | 2-3 天     | ✅ 已完成 |
| Phase 4     | C_WorkFlow                                                          | 1-2 天     | ✅ 已完成 |
| **Phase 5** | **C_Draggable + C_Tree + C_GlobalSearch + C_FullCalendar + C_Time** | 2 天       | ✅ 已完成 |
| Phase 6     | P1 剩余组件批量清理                                                 | 1-2 天     | ⏳ 待开始 |
| Phase 7     | P2 小改 + console 全局清理                                          | 1 天       | ⏳ 待开始 |
