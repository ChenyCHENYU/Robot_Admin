...
#### 1. ✅ form-validate（表单验证）- 已迁移

**原路径**: ~~`src/utils/v_verify.ts`~~ (已删除)  
**新包名**: `@robot-admin/form-validate`  
**版本**: v2.0.0  
**状态**: ✅ 已发布到 npm  
...
| 包名                           | 来源                                       | 状态 | 版本  | 备注               |
| ------------------------------ | ------------------------------------------ | ---- | ----- | ------------------ |
| `@robot-admin/form-validate`   | ~~`src/utils/v_verify.ts`~~（已删除）       | ✅   | 2.0.0 | 已发布到 npm       |
| `@robot-admin/hooks`           | `src/hooks/{useDownload,useExcel,useCopy}` | ⏳   | -     | 待拆分             |
...
- [x] 安装 `@robot-admin/form-validate` v2.0.0
- [x] 配置 bun link 本地开发
- [x] 迁移 15 个文件的导入路径
- [x] 删除 `src/utils/v_verify.ts`
- [x] 全局功能测试通过
- [ ] 安装 `@robot-admin/hooks`（待拆分）
