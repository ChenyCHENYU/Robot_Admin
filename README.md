...
你正在查看的是 **`micro-app` 分支**，这是 Robot Admin 多架构系列中的**微前端版本**。
...
子应用是**独立的 Git 仓库**，与主应用平级部署。主应用通过 `envs/.env.*` 中的 `VITE_MICRO_<APP>_URL` 环境变量获取子应用地址：
...
**场景 1：企业数字化平台**
主应用统一认证+权限+门户，各业务系统（ERP/OA/CRM/WMS）独立接入，统一入口管理。

**场景 2：遗留系统零成本改造**
现有老旧系统（React/jQuery/Vue 2）无需重构，在门户直接注册地址即可接入，渐进式现代化。

**场景 3：多团队独立协作**
各团队独立维护子应用仓库，主应用负责集成和统一导航，独立发版互不干扰。
...
- [ ] `envs/.env.production` 中 `VITE_MICRO_*_URL` 已填写生产地址
