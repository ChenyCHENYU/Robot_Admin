...
- **data:** 认证与业务数据源分离配置；生产、预发环境强制使用远端数据，账号、字典、菜单、角色、用户及权限治理页面接入统一 Remote/Mock 契约。
- **requests:** 用户与角色列表采用“仅最新请求生效”策略，主动取消过期请求，避免快速搜索、翻页时旧响应覆盖新状态。
- **quality:** 清理非演示代码中的宽泛类型与通配声明，增加环境、安全头、错误上报、菜单边界、并发取消及构建体积预算测试。
- **accessibility:** 导航分组、门户快捷入口、标签页、折叠面板和 About 卡片补齐原生交互语义、键盘焦点与 ARIA 状态。
- **performance:** 代码高亮改为页面级懒初始化，并通过入口、预加载、样式、首屏总量及最大异步 chunk 预算阻止体积回归；本地组件源码模式复用应用侧 VisActor 运行时，消除约 2.1 MiB 重复 chunk。
...
- **security:** 增加 CSP、HSTS、Referrer Policy、Permissions Policy 等部署响应头；客户端错误上报仅允许同源端点并自动脱敏凭据、个人信息与 URL 查询参数。
- **account:** 修复确认密码校验读取错误上下文、登录完成回调携带明文密码，以及远端模式首屏短暂展示演示账号数据的问题。
- **permissions:** 权限列表统一使用 `/sys/permissions` 并按 Axios 配置传递查询参数，新增数据权限、临时授权、约束与审计的远端读写闭环。
- **regression:** 恢复登录页记住我、忘记密码、人机验证、验证码、扫码、第三方登录、注册入口及演示凭据；恢复代码高亮和跨刷新认证状态。
- **styles:** 修复全局搜索、消息中心、语言、主题、功能引导以及手动导入的表格/树/图标组件样式丢失，补齐 Driver.js 基础样式和组件包 UnoCSS 图标白名单。
- **navigation:** 动态路由只等待菜单主契约，辅助按钮与数据权限在默认拒绝策略下后台补齐，避免接口延迟长期占用顶部进度条。
- **types:** 删除遮蔽 Naive UI 的通配声明并修复真实应用与 Vite 配置类型错误，类型门禁同时覆盖应用和 Node 配置。
- **management:** 补齐菜单、字典和权限资源的可持久 Mock CRUD，菜单拖拽真实更新层级与排序；重置复用表单的隐藏 ID，避免新增操作误更新旧记录。
- **resilience:** 角色状态、删除、保存与权限分配补齐失败收敛和用户反馈，远端删除成功后始终刷新列表。
- **startup:** 普通开发命令默认跳过会同步执行多条 Git 查询的控制台横幅，热启动恢复至约 3 秒；保留 `dev:banner` 作为显式调试入口。
- **runtime:** 修复登录后首次加载布局及深层演示页面时，Vite 依赖重新发现导致的强制刷新、CommonJS 默认导出不兼容和路由中断；重量级页面继续保持路由级按需加载。
- **icons:** 恢复历史 `<Icon>` 模板到 `@iconify/vue` 的直接解析，并用精确白名单覆盖组件包运行时生成的 17 个 UnoCSS 图标类，不恢复已废弃的整套图标插件链。
- **login:** 为登录容器补齐独立于 UnoCSS 生成时序的 `#181818` 背景兜底，确保开发、生产和样式按需加载场景下呈现一致。
- **login:** 恢复人机验证拼图图标和空文本图标模式；登录页空闲预热认证壳层，并在提交期间暂停 3D 渲染，避免布局解析与 Spline 争抢主线程。
- **guide:** 未显式配置遮罩透明度时保留 Driver.js 默认值，修复首次打开全黑；组件关闭后清理遮罩和全局状态。
- **navigation:** 菜单叶子节点增加无副作用的意图预取，移除分组菜单与宿主同时 `router.push` 的重复跳转，首个认证页面的关键 UnoCSS 在构建期稳定生成。
- **performance:** 组件库运行时代码统一使用深层按需入口，纯类型引用统一使用 `import type`；修复级联选择器首次进入意外加载整套组件库及数千个开发态模块的问题。
- **verification:** 新增验证码、组合样式、首个认证帧、菜单意图和引导默认值回归契约；以全新存储和正式发布包复检登录、跨刷新导航、全局搜索、消息中心及功能引导。
