...
- **components:** 升级至 `@robot-admin/naive-ui-components@0.11.6`，使用官方 resolver、子路径入口与按需基础样式，移除旧的 CSS 兼容探测插件，并接入 C_Map 强类型实例与高德安全配置。
...
- **verification:** 新增 UI 能力与样式回归契约；完成 127 条可访问路由浏览器遍历、登录与跨刷新导航、全局搜索、消息中心及功能引导的交互复检。
